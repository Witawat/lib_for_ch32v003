...
void Delay_Init(void) {
  ...
  // Config SysTick for 1ms Interrupt (Continuous)
  SysTick->CTLR = 0;
  SysTick->SR = 0;
  SysTick->CNT = 0;
  SysTick->CMP = p_ms; // 1ms Target

  // Enable Counter, Interrupt, HCLK/8 (Bit 2=0 for External/8, Bit1=1 for Int,
  // Bit0=1 Enable) CH32V003 SysTick CTLR: [0]STE [1]STIE [2]STCLK ... We strive
  // for compat with standard WCH defines if available, else usage bits
  // directly. Based on WCH example:
  SysTick->CTLR = (1 << 1) | (1 << 0); // IE | ENABLE (Clock default /8)
...
void SysTick_Handler(void) {
  SysTick->SR = 0;      // Clear Flag
  SysTick->CMP += p_ms; // Next Compare Target (Continuous)
  g_millis++;
}
...
void Delay_Us(uint32_t n) {
  ...
  uint32_t target = n * p_us;

  // Handle wrap around by using unsigned subtraction
  while ((SysTick->CNT - now) < target)
    ;
}

void Delay_Ms(uint32_t n) {
  uint32_t start = g_millis;
  while ((g_millis - start) < n)
    ;
}
